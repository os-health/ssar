...
　　作为一款优秀的sar类型工具，ssar在各种资源开销上都做了很多大量的优化。这里分别对CPU和内存的性能开销，以及存储开销情况做一些介绍。

<a name="性能开销"></a>

### 16.1、性能开销

　　关于ssar本身的性能开销，我们可以用如下ssar的命令进行观察。

```bash
$ ssar proc -p $(pidof sresar) -i 1 --cpu --mem

collect_datetime     pcpu   rss    cmd
2023-04-09T11:40:00   0.7   1524   sresar
2023-04-09T11:41:00   1.0   1708   sresar
2023-04-09T11:42:00   0.5   3552   sresar
2023-04-09T11:43:00   0.3   3552   sresar
2023-04-09T11:44:00   0.5   3552   sresar
2023-04-09T11:45:00   0.3   3552   sresar

```

　　经对生产环境进行观察，在万台以上规模服务器上的运行结果，CPU开销最大不高于单核3.0%，内存开销最大不高于4.0MB。这2个指标表明ssar的性能开销是非常低的。

　　考虑到具体的业务特点不同，实际的性能开销可能与此略有出入。就ssar而言，影响它CPU开销的主要因素是进程数量，影响内存开销的主要因素是进程全名称的长度。
...
　　ssar不但有极低的内存使用开销，它还对内存使用过程做了极致的优化。当采集进程启动后，不但物理内存(rss)使用基本保持恒定不变，次缺页中断指标(min_flt)也基本保持0增长状态。这极大的增强了ssar在系统内存枯竭情况下的鲁棒性，可以比其他监控系统更能记录到最后时刻的关键数据。

```bash
$ ssar proc -p $(pidof sresar) -i 1 --mem

collect_datetime     rss  min_flt  cmd
2023-04-09T11:40:00  1524     631  sresar
2023-04-09T11:41:00  1708    1231  sresar
2023-04-09T11:42:00  3552    1749  sresar
2023-04-09T11:43:00  3552    1749  sresar
2023-04-09T11:44:00  3552    1749  sresar
2023-04-09T11:45:00  3552    1749  sresar
```
...
　　ssar工具巨大的指标数量，必然伴随大量的磁盘存储空间占用。ssar工具默认是记录的7天历史数据，统计数据显示大多数情况下不会占用超过1GB数据。

　　ssar工具的配置文件中有个参数，可以设置当磁盘空间使用率达到90%之后，ssar就停止采集，这样保证不会将磁盘空间占满的情况发生。更巧妙的是当磁盘空间已经到90%，如果同时还有其他程序还在持续记录数据，那么磁盘空间使用率很可能就会继续上涨到91%、92%、93%或94%。此时，ssar会将已经记录的历史数据从最老的开始删除，尽量降低磁盘空间占用情况。
